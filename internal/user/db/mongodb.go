...
func (d *db) Create(ctx context.Context, user user.User) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to create user ude to error: %v", err)
	}
	...
	d.logger.Trace(user)
	return "", fmt.Errorf("failed to convert ojectid to hex. probably oid: %s", oid)
}
...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if result.Err() != nil {
		if errors.Is(mongo.ErrNoDocuments, mongo.ErrNoDocuments) {
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to unmarhsal user bytest. error: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to execute update user query. error: %v")
	}
...
func (d *db) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to exucete query. error: %v", err)
	}
